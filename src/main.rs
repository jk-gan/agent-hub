...
use gpui::prelude::*;
use gpui::{
    App, Application, Bounds, ClipboardEntry, Context, Entity, ExternalPaths, FocusHandle,
    ImageFormat, KeyBinding, Menu, MenuItem, PathBuilder, Pixels, Render, ScrollHandle,
    Subscription, Window, WindowBounds, WindowOptions, actions, canvas, div, point, px, size,
};
use gpui_component::Root;
use gpui_component::button::{Button, ButtonVariants as _};
use gpui_component::highlighter::HighlightTheme;
use gpui_component::input::{
    Enter as InputEnter, InputEvent, InputState, MoveDown as InputMoveDown, MoveUp as InputMoveUp,
    Position,
};
...
impl AppShell {
    ...
    fn append_live_tool_output_delta(
        ...
        let normalized_delta = Self::normalize_tool_output_text(delta);
    ...
    fn normalize_tool_output_text(text: &str) -> String {
        text.replace("\r\n", "\n")
            .replace('\r', "\n")
            .chars()
            .filter(|ch| *ch != '\0')
    ...
    fn fenced_code_block(language: Option<&str>, content: &str) -> String {
        let fence_len = max(3, Self::longest_char_run(content, '`') + 1);
    ...
    fn format_command_execution_item(item: &Value) -> Option<RenderableMessage> {
        ...
        let output = item
            ...
            .and_then(Value::as_str)
            .unwrap_or_default();
        let output = Self::normalize_tool_output_text(output);
        ...
        let mut command_output = format!("$ {command}");
        if !output.is_empty() {
            command_output.push('\n');
            command_output.push_str(output);
        }
        let mut body = Self::fenced_code_block(Some("bash"), &command_output);
    ...
    fn renderable_messages_from_thread_response(
        ...
        if was_truncated {
            messages.push(RenderableMessage::Text {
                speaker: ThreadSpeaker::Assistant,
                content: Self::THREAD_TRUNCATION_NOTICE.to_string(),
                image_refs: Vec::new(),
            });
        }
    ...
    pub(crate) fn set_command_execution_expanded(
        &mut self,
        message_ix: usize,
        expanded: bool,
    ) -> bool {
        let Some(message) = self.selected_thread_messages.get_mut(message_ix) else {
            return false;
        };
    ...
    pub(crate) fn set_file_change_expanded(&mut self, message_ix: usize, expanded: bool) -> bool {
        let Some(message) = self.selected_thread_messages.get_mut(message_ix) else {
            return false;
        };
    ...
    pub(crate) fn set_web_search_expanded(&mut self, message_ix: usize, expanded: bool) -> bool {
        let Some(message) = self.selected_thread_messages.get_mut(message_ix) else {
            return false;
        };
    ...
    fn render_pending_approval_panel(
        ...
        components::pending_approval_panel::render(
            self,
            text_color,
            subtext_color,
            overlay_color,
        ...
        let root = div()
                                ...
                                .on_click({
                                    ...
                                    move |_, _, cx| {
                                        let _ = this.update(cx, |view, cx| {
                                            ...
                                            view.start_new_thread_in_workspace(workspace_cwd, cx);
                                        });
                                    }
                                }),
                        ),
                                ...
                                |this, limits| {
                                    this.child(
                                        div()
                                            ...
                                            .bg(mantle)
                                            .when_some(limits.primary.as_ref(), |this, entry| {
                                            ...
                                            .when_some(limits.secondary.as_ref(), |this, entry| {
                                                let label = Self::format_window_label(
                                                    entry.window_duration_mins,
                                                );
                                                this.child(Self::render_rate_limit_bar(
            ...
            .child(
                div()
                        ...
                        div()
                            ...
                            .child({
                                ...
                                div()
                                    ...
                                    .items_center()
                                    .gap(px(8.))
                                    .when(sidebar_collapsed, |this| this.pl(px(80.)))
                                    .child(
                    ...
                    .child(components::thread_timeline_panel::render(
                        self,
                    ...
                    .child(components::composer_panel::render(
                        self,
                        text_color,
                        subtext_color,
                        overlay_color,
